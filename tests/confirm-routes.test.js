...
describe("confirm route", () => {
  ...
  it("emits confirmation resolution with the pending session path when available", async () => {
    const confirmStore = {
      peek: vi.fn(() => ({ sessionPath: "/sessions/current.jsonl" })),
      resolve: vi.fn(() => true),
    };
    const engine = { emitEvent: vi.fn() };
    const app = new Hono();
    ...
    const res = await app.request("/api/confirm/test-confirm-id", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "confirmed_once" }),
    });
