#!/usr/bin/env node
/**
 * V8 题库测试 — 持久 WebSocket 版
 *
 * 与 codex-v8-short-lived.mjs 的区别:
 *   - 整个 session 用一条持久 WS（模拟真 Lynn UI）
 *   - 每题等 retry 完成后再发下一个（不丢 internal retry 的 text）
 *   - 支持 --lynn-home / --server-info，便于打临时 server
 *
 * Usage:
 *   node tests/benchmarks/v8-persistent.mjs
 *   node tests/benchmarks/v8-persistent.mjs --quick
 *   node tests/benchmarks/v8-persistent.mjs --server-info ~/.lynn-dev/server-info.json
 */
...
Options:
  ...
  --lynn-home PATH       default: ~/.lynn or LYNN_HOME
...
const lynnHome = expandHome(args.lynnHome);
const serverInfoPath = expandHome(args.serverInfo || path.join(lynnHome, "server-info.json"));
const info = JSON.parse(fs.readFileSync(serverInfoPath, "utf-8"));
const WS_URL = `ws://127.0.0.1:${info.port}/ws`;
const TOKEN = info.token;
const outputDir = expandHome(args.output || path.join("output", `v8-persistent-${nowStamp()}`));

const TURN_GAP_MS = 2000;
const TURN_TIMEOUT_MS = 120_000;
const RETRY_IDLE_MS = 3000;
...
function sendPrompt(ws, text) {
  ws.send(JSON.stringify({ type: "prompt", text }));
}

function waitForTurnDone(ws, timeoutMs = TURN_TIMEOUT_MS) {
  return new Promise((resolve) => {
    ...
    const idleTimer = setInterval(() => {
      if (turnEndCount > 0 && !streamOpen && Date.now() - lastEventTime > RETRY_IDLE_MS) {
        cleanup();
        resolveResult("idle");
      }
    }, 500);
    ...
    const timeoutTimer = setTimeout(() => {
      cleanup();
      errors.push(`timeout ${timeoutMs}ms`);
    ...
    function handler(data) {
      try {
        ...
        if (msg.type === "text_delta") {
          ...
          hadToolCall = true;
          tools.push({ name: msg.name || "unknown", success: null });
        } else if (msg.type === "tool_end") {
          const tool = [...tools].reverse().find((item) => item.name === msg.name && item.success == null);
          if (tool) tool.success = msg.success !== false;
        } else if (msg.type === "turn_retry") {
          hadRetry = true;
          streamOpen = true;
          retryReasons.push(msg.reason || "unknown");
        } else if (msg.type === "status" && typeof msg.isStreaming === "boolean") {
          sawStatus = true;
          streamOpen = msg.isStreaming;
        } else if (msg.type === "error") {
          errors.push(msg.message || "unknown error");
        } else if (msg.type === "turn_end") {
          turnEndCount++;
          if (!sawStatus) streamOpen = false;
        }
      } catch (error) {
...
async function main() {
  ...
  console.log(`V8 Persistent WS Test — ${allCases.length} cases${args.quick ? " (quick)" : ""}`);
  ...
  for (const testCase of allCases) {
    process.stdout.write(`[${testCase.id.padEnd(8)}] ${testCase.name.padEnd(14)} ... `);
    sendPrompt(ws, testCase.prompt);
    const result = await waitForTurnDone(ws);
    const warnings = [];
  ...
  fs.writeFileSync(path.join(outputDir, "summary.md"), [
    ...
    ...results.map((item) => [
      ...
      item.textChars,
      item.turnEndCount,
      item.retryReasons.length ? item.retryReasons.join(",") : (item.hadRetry ? "yes" : ""),
      item.tools.map((tool) => `${tool.name}:${tool.success === false ? "fail" : tool.success === true ? "ok" : "?"}`).join(", "),
      item.ttftMs ?? "",
      item.elapsedMs,
      item.warnings.join(", "),
    ].join(" | ")).map((line) => `| ${line} |`),
  ...
  console.log(`\n=== ${pass}/${pass + fail} passed · warnings=${warn} ===`);
  console.log(`Results: ${path.join(outputDir, "summary.md")}`);
