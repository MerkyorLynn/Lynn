...
## 🔭 V0.80 方向预告:GUI + CLI Worker Fleet

V0.80 会把 Lynn 带回编程主战场,但不是再做一个单 CLI 或 IDE 插件。Lynn 的方向是把 **GUI 变成多个 CLI Agent 的指挥台**:你可以在图形界面里拆任务、派发给不同 CLI worker、查看日志和 diff、跑门禁、合并或丢弃结果。

这不是“不服务代码”。恰恰相反,Lynn 要把代码任务和业务任务放到同一个调度系统里:

- **GUI 指挥台**:在 Lynn 桌面端创建任务、分配 worker、查看每条 worker 的 stdout/stderr、diff、测试结果和越界修改。
- **CLI Worker Fleet**:统一调度 Codex、Claude Code、Qwen、codebuddy 或自定义 CLI,每个 worker 自动进入独立 worktree。
...
- **`@lynn/cli`**:V0.80 同步发布 npm CLI 包,支持 `lynn -p`、`lynn code`、`lynn doctor` 和 `lynn worker run`,既能给终端用户使用,也能被 GUI 调度。
...
| 格式 | 大小 | 适合谁 |
|---|---|---|
| **BF16 全精度** | 65 GB | 微调 / 二次训练起点 |
| **NVFP4-v8-RTN** | 21 GB | 单卡 24 GB GPU(SGLang dev-cu13 nightly 已验) |
| **Q4_K_M GGUF** | 20 GB | llama.cpp / Ollama,消费级显卡能跑 |
...
- 🇨🇳 **ModelScope**(国内下载推荐):[`Merkyor/Lynn-V4-Pro-Distill-Qwen-35B-A3B`](https://modelscope.cn/models/Merkyor/Lynn-V4-Pro-Distill-Qwen-35B-A3B)([Q4_K_M](https://modelscope.cn/models/Merkyor/Lynn-V4-Pro-Distill-Qwen-35B-A3B-Q4_K_M) · [NVFP4-v8-RTN](https://modelscope.cn/models/Merkyor/Lynn-V4-Pro-Distill-Qwen-35B-A3B-NVFP4-v8-RTN))

工具链全开源:[lynn-distill-toolkit](https://github.com/MerkyorLynn/lynn-distill-toolkit)(4-gate eval / sanity / pipeline / 剪枝)+ [qwen3.6-nvfp4-toolkit](https://github.com/MerkyorLynn/qwen3.6-nvfp4-toolkit)(NVFP4 量化工具链)。

### 接下来还会出什么

- 🔜 **Lynn-V4-Flash-Distill**(5/14 ETA)— 更轻的 Flash 变体,主打速度
- 🔜 **Lynn-27B-A3B**(5 月底)— MoE 剪枝版,单 4090 / 5060 Ti 跑得动
- 🔜 **Lynn Engine** — Lynn 专属推理引擎。直接读 NVFP4 + MoE 原生 layout,绕开 transformers / vLLM 上游集成节奏。Phase 1-4 路线:loader → BF16 慢路径 oracle → layer parity → 5-token parity → 原生 FP4 GEMM 性能
...
T4  智谱 GLM-5-Turbo / GLM-5.1（coding plan）
T5  Kimi K2.6（api.kimi.com coding plan，256K 窗口）
T6  Step-3.5 Flash / MiniMax M2.7-highspeed（末级兜底）
```
...
**macOS（Apple Silicon / Intel）**：从 [Releases](https://github.com/MerkyorLynn/Lynn/releases) 下载最新 `.dmg`。V0.79.9 的 Apple Silicon / Intel DMG 均已签名、公证、stapled,并通过 Gatekeeper 校验。

**Windows**：从 [Releases](https://github.com/MerkyorLynn/Lynn/releases) 下载最新 `.exe`，直接运行。

> **Windows SmartScreen 提示：** V0.79.9 安装包已完成代码签名；首次运行仍可能因为新版应用声誉积累不足出现 SmartScreen 确认提示。
...
| 平台 | 状态 |
|------|------|
| macOS (Apple Silicon) | 已支持（V0.79.9 签名 + 公证 DMG） |
| macOS (Intel) | 已支持 |
| Windows x64 | Beta |
| Linux | 计划中 |
| 移动端 (PWA) | 计划中 |
...
**能**。Quick Start 60 秒进主界面直接聊，全程零配置。后台自动走 Brain v2 降级链（MiMo ⭐ → Spark Qwen3.6-35B → DeepSeek V4 → GLM-5 → Kimi K2.6 → Step / MiniMax），哪档有空走哪档。

### Q5：Windows 能用吗？

可以。V0.79.9 的 **Windows 安装包已完成代码签名**，但 SmartScreen 仍可能因为新版应用声誉积累不足而提示确认；macOS Apple Silicon / Intel DMG 均已签名、公证并通过 Gatekeeper 校验。
