...
用 `Lynn code -p "<任务>" --json --cwd <worktree>` 启动无交互编码任务,从 stdout 读取 JSONL 事件;需要无人值守改文件并运行测试时,在隔离 git worktree 里加 `--approval yolo --sandbox danger-full-access`。
...
- `--approval yolo --sandbox danger-full-access` 只用于隔离 git worktree。`workspace-write` 是人类守护模式,可能阻断 worker 自测需要的 shell 命令。
...
| 受限实验 | `--approval yolo --sandbox workspace-write` |

ask 模式会弹授权卡片。yolo 模式不会逐条询问,适合黑灯工厂。隔离靠外层 worktree,Fleet gate 负责 ownership、forbidden globs、测试结果和最终 diff;不要用 `workspace-write` 饿住 worker 的自测 shell。
