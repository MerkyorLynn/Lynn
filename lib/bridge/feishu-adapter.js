...
let _larkModule = null;
async function loadLarkSDK() {
  if (!_larkModule) {
    try {
      _larkModule = await import("@larksuiteoapi/node-sdk");
    } catch (err) {
      throw new Error(
        `飞书 SDK (@larksuiteoapi/node-sdk) 未安装。请运行 npm install @larksuiteoapi/node-sdk 后重试。原始错误: ${err.message}`
      );
    }
  }
  return _larkModule;
 ...
 */
export async function createFeishuAdapter({ appId, appSecret, onMessage, onStatus }) {
