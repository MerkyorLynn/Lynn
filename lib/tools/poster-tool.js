    ...
    execute: async (_toolCallId, params) => {
      ...
      const contentHtml = (params.content || "")
        ...
        .map((line) => {
          ...
          if (line.startsWith("# ")) return `<h2 style="font-size:28px;font-weight:800;margin:24px 0 12px;color:${T.accent}">${escapeHtml(line.slice(2))}</h2>`;
          if (line.startsWith("## ")) return `<h3 style="font-size:22px;font-weight:700;margin:20px 0 8px;color:${T.text}">${escapeHtml(line.slice(3))}</h3>`;
          if (line.startsWith("- ")) return `<p style="font-size:18px;margin:6px 0;padding-left:20px;color:${T.text}">• ${escapeHtml(line.slice(2))}</p>`;
          return `<p style="font-size:18px;line-height:1.8;margin:8px 0;color:${T.text}">${text}</p>`;
        })
        .join("\n");
...
${params.subtitle ? `<p class="subtitle">${subtitle}</p>` : ""}
<div class="divider"></div>
<div class="content">${contentHtml}</div>
${params.footer ? `<div class="footer">${footer}</div>` : ""}
</div></div></body></html>`;
