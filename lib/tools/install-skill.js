...
export function createInstallSkillTool({ agentDir, getConfig, resolveUtilityConfig, onInstalled }) {
  return {
    ...
    execute: async (_toolCallId, params) => {
      ...
      const allowGithub = learnCfg.allow_github_fetch !== false;
      ...
      if (github_url?.trim()) {
        ...
        try {
          const apiRes = await fetch(`https://api.github.com/repos/${owner}/${repo}`, {
            headers: { "User-Agent": "LynnBot/1.0", Accept: "application/vnd.github.v3+json" },
        ...
        for (const candidate of [...new Set(candidates)]) {
          ...
          try {
            const r = await fetch(rawUrl, {
              headers: { "User-Agent": "LynnBot/1.0" },
              signal: AbortSignal.timeout(GITHUB_API_TIMEOUT),
            });
            if (r.ok) {
        ...
        // 6. Safety scan: reject dangerous patterns in skill content
        ...
        for (const pat of _dangerousPatterns) {
          if (pat.test(content)) {
            return {
              content: [{ type: "text", text: t("error.installSkillDangerousContent") || "Skill content contains potentially dangerous patterns and was rejected." }],
              details: { rejected: true, pattern: pat.source },
            };
          }
        }

        // 7. 写入 learned-skills/{name}/SKILL.md
        ...
        return {
          ...
          details: { skillName: name, stars, source: "github", safetyReview: safetyPassed },
        };
      }
      ...
      return {
        ...
        details: { skillName: name, source: "content", safetyReview: safetyPassed2 },
      };
    },
  };
}
