      ...
      try {
        ...
        if (contentType.includes("application/json")) {
          ...
          if (text.length < MIN_USEFUL_HTML_TEXT) {
            try {
              const reader = await fetchViaJinaReader(currentUrl, maxLen);
              text = reader.text;
              format = reader.format;
              details = reader.details;
            } catch {
              // 直抓结果虽短，但也可能是有效页面，这里不直接报错
            }
          }
        } else {
          text = raw;
        ...
        try {
          const maxLen = params.maxLength ?? MAX_CONTENT_LENGTH;
          const reader = await fetchViaJinaReader(url, maxLen);
          const fallbackHeader = t("error.fetchSource", {
            host: parsedUrl.hostname,
            path: parsedUrl.pathname,
            format: reader.format,
          });
          return {
            content: [{ type: "text", text: fallbackHeader + reader.text }],
