...
export function getNewMessages(filePath, bookmark, selfName) {
  ...
  // 跳过自己发的（支持数组，兼容 agentId + agentName 两种 sender）
  if (selfName) {
    const selfNames = Array.isArray(selfName) ? selfName : [selfName];
    filtered = filtered.filter(m => !selfNames.includes(m.sender));
  }
...
export function getRecentMessages(filePath, count = 10, selfName) {
  ...
  let filtered = selfName
    ? (() => { const names = Array.isArray(selfName) ? selfName : [selfName]; return messages.filter(m => !names.includes(m.sender)); })()
    : messages;
...
export function setChannelArchived(filePath, archived = true) {
  ...
  rewriteFrontmatter(filePath, (meta) => {
    const prevArchived = meta.archived === true || meta.archived === "true";
    if (prevArchived === archived) return false;
    changed = true;
    if (archived) {
      meta.archived = "true";
      meta.archivedAt = formatTimestamp(new Date());
    } else {
 ...
 * 内置 token 预算控制：从最新消息开始倒序填充，超预算则截断旧消息。
 * 单条消息超过 maxCharsPerMsg 时自动截断并标注。
 *
 * @param {Array<{sender: string, timestamp: string, body: string}>} messages
 * @param {object} [opts]
 * @param {number} [opts.tokenBudget=4000] - 总 token 预算（1 token ≈ 2 中文字符）
...
export function formatMessagesForLLM(messages, { tokenBudget = 4000, maxCharsPerMsg = 800 } = {}) {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (body.length > maxCharsPerMsg) {
      body = body.slice(0, maxCharsPerMsg) + truncLabel;
    }
    const line = `[${m.timestamp}] ${m.sender}: ${body}`;
    if (totalChars + line.length > charBudget && result.length > 0) break;
    result.unshift(line);
  ...
  if (result.length < messages.length) {
    ...
    const hint = getLocale().startsWith("zh")
      ? `(已省略更早的 ${skipped} 条消息)`
