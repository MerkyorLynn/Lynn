...
export function createChannelTicker({
  ...
  function collectAgentChannels(agentId) {
    ...
    try {
      const channelFiles = fs.readdirSync(channelsDir).filter((file) => file.endsWith(".md"));
      for (const file of channelFiles) {
        const channelName = file.replace(/\.md$/, "");
        if (bookmarks.has(channelName)) continue;
        const meta = getChannelMeta(path.join(channelsDir, file));
    ...
    for (const [channelName, bookmark] of bookmarks) {
      ...
      // 用 bookmark 判断有没有新消息
      const hasNew = getNewMessages(channelFile, bk, self).length > 0;
      // 但上下文用滑动窗口（最近 10 条）
      const recentMsgs = hasNew ? getRecentMessages(channelFile, 20, self) : [];
  ...
  function triggerImmediate(channelName, { mentionedAgents } = {}) {
    ...
    _triggerChain = _triggerChain.then(async () => {
      ...
      // 启动新的 triage
      const channelFile = path.join(channelsDir, `${channelName}.md`);
      const triggerMessage = getLatestMessage(channelFile);
      _triagePromise = _doTriage(channelName, { mentionedAgents, triggerMessage });
      await _triagePromise.catch(() => {});
      _triagePromise = null;
    }).catch(() => {});
  ...
  async function _doTriage(channelName, { mentionedAgents, triggerMessage } = {}) {
    ...
    try {
      for (const agentId of agents) {
        ...
        const channelsMdPath = path.join(agentsDir, agentId, "channels.md");
        if (channelMembers.includes(agentId)) {
          const currentBookmarks = readBookmarks(channelsMdPath);
          if (!currentBookmarks.has(channelName)) {
            addBookmarkEntry(channelsMdPath, channelName);
            debugLog()?.log("ticker", `healed missing triage bookmark: ${agentId}/#${channelName}`);
          }
        }
        const bookmarks = readBookmarks(channelsMdPath);
        ...
        const recentMsgs = getRecentMessages(channelFile, 20, selfNames(agentsDir, agentId));
        ...
        try {
          const result = await executeCheck(agentId, channelName, recentMsgs, [], { signal, triggerMessage });
      ...
      // ── 4.5 所有 agent 回复完后，触发主持人总结 ──
      if (!signal.aborted && onAllReplied) {
        try {
