...
function buildJianPrompt({ dirPath, jianContent, files, jianChanged, filesChanged, isZh }) {
  ...
  parts.push(isZh
    ? [
        ...
        "遇到 `- [ ]` 待办时，优先判断是否可以直接完成；完成后请更新笺并写下很短的结果。",
        "已经标了 `- [x]` 的任务是已完成的，跳过不要重复执行。",
        "如果待办里已经标了 `⏰ 已设定`，说明自动任务已经建好，不要重复创建。",
        "如果笺里出现每天 / 工作日 / 每周 这类时间安排，优先把它视作自动任务，而不是一次性聊天请求。",
        "涉及删除 / 移动 / 重命名 / 批量覆盖等非读取操作时，先把执行计划写回笺里，等下一轮巡检再执行。",
      ].join("\n")
    : [
        "Follow the jian instructions. If no action is needed, do not call any tools.",
        "When you see `- [ ]` todos, first decide whether they can be completed directly; if completed, update the jian and leave a very short result.",
        "Tasks already marked `- [x]` are completed — skip them, do not re-execute.",
        "If a task already has `⏰ scheduled`, the automation already exists — do not create it again.",
        "When the jian contains phrases like every day / weekdays / weekly, prefer treating them as scheduled tasks instead of one-off chat asks.",
        "For delete / move / rename / overwrite style actions, write a short execution plan back into the jian first, then wait until the next patrol before executing.",
      ].join("\n"));
...
export function hasActionableJianContent(content, locale = "zh") {
  ...
  const activeBody = body
    .split("\n")
    .filter((line) => !/^- \[ \].*(?:⏰|已设定|scheduled|automation|自动任务)/i.test(line.trim()))
    .join("\n");
  ...
  return actionableRe.test(compact);
}
...
export function createHeartbeat({
  ...
  async function _doBeat() {
    try {
      ...
      const firstDeskScan = !_lastDeskFingerprint;
      const deskChanged = !firstDeskScan && deskFingerprint !== _lastDeskFingerprint;
      ...
      // ── Phase 1.5: 插件工作区扫描（v0.77） ──
      try {
        const bridge = getPluginBridge();
        if (bridge) {
          const pluginChanges = bridge.heartbeatScan();
          if (pluginChanges.length > 0) {
            const summary = isZh
              ? `插件工作区: ${pluginChanges.map(c => c.pluginId).join("、")} 有变化`
              : `Plugin workspaces: ${pluginChanges.map(c => c.pluginId).join(", ")} changed`;
            console.log(`${tag}  Phase 1.5: ${summary}`);
            devlog(`Phase 1.5: ${summary}`);
          }
        }
      } catch (err) {
  ...
  function _detectJianChanges(jianDirs) {
    ...
    for (const dir of jianDirs) {
      ...
      const changed = jianChanged || filesChanged;
      if (!changed) continue;

      const actionable = hasActionableJianContent(dir.jianContent, locale);
      if (!actionable) {
        registry[key] = {
          jianHash,
          filesHash,
          lastCheckedAt: new Date().toISOString(),
          lastSkippedReason: "quiet-no-actionable-jian",
        };
    ...
    if (registryTouched) saveRegistry(registry);
        ...
        try {
          ...
          for (const task of recurringTasks) {
            try {
              const created = await onJianSchedule({
                ...
                label: task.taskText.slice(0, 32),
              });
              if (created) {
                createdTasks.push({
                  ...task,
          ...
          if (createdTasks.length > 0) {
            ...
            const preview = createdTasks
              ...
              .map((task) => {
                const nextRun = task.nextRunAt
                  ? new Date(task.nextRunAt).toLocaleTimeString(isZh ? "zh-CN" : "en-US", {
                    hour: "2-digit",
                    minute: "2-digit",
                    hour12: false,
                  })
                  : "";
                return nextRun
                  ? `${task.taskText}（${isZh ? "下次" : "next"} ${nextRun}）`
                  : task.taskText;
              })
              .join(isZh ? "、" : ", ");
            dir.jianContent = upsertRecentExecutionSection(dir.jianContent, {
              ...
              type: "cron",
              label: isZh ? "自动任务设定" : "Automation setup",
              at: Date.now(),
              locale,
            });
            fs.writeFileSync(path.join(dir.absPath, "jian.md"), dir.jianContent, "utf-8");
            dir.jianHash = quickHash(dir.jianContent);
