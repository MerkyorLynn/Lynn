    ...
    try {
      ...
      if (userProfile) {
        try {
          ...
          const stats = extractSessionStats(sessionPath);
          if (stats) userProfile.updateFromSession(stats);
        } catch (err) {
          console.error(`\x1b[90m[memory-ticker] user profile update failed: ${err.message}\x1b[0m`);
        }
      }
    } catch (err) {
