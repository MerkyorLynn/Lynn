...
export class FactStore {
  ...
  /**
   * 合并 tag + FTS 搜索结果，简单评分排序（Phase 1 主动召回用）
   *
  ...
  searchCombined(keywords, limit = 5) {
    ...
    try {
      ...
      for (const r of tagResults) {
        seenIds.add(r.id);
        scored.push({ row: r, score: (r.matchCount || 1) * 2 });
      }
    } catch {}

    // 策略 2：FTS 全文搜索补充（tag 命中 < 2 时）
    if (scored.length < 2) {
