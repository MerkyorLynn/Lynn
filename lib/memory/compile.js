 ...
 *   compileFacts()    → facts.md（重要事实，优先保留高重要度事实）
 *
...
async function withFileLock(name, fn) {
  const lockDir = path.join(os.tmpdir(), `.lynn-lock-${name}`);

  while (_activeLocks.has(name)) {
    await _activeLocks.get(name).catch(() => {});
  }

  let release;
  const lockPromise = new Promise((resolve) => { release = resolve; });
  _activeLocks.set(name, lockPromise);
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (i === maxRetries - 1) throw new Error(`compile lock timeout: ${name}`);
...
function parseFactsMarkdown(text) {
  return String(text || "")
    .split(/\n+/)
...
function buildHighImportanceFactSection(facts) {
  if (!facts || facts.length === 0) return "";
  return facts.map((fact, index) => {
    const score = Number(fact.importance_score || 0).toFixed(1);
    const hits = Number(fact.hit_count || 0);
    const tags = Array.isArray(fact.tags) && fact.tags.length > 0 ? ` (${fact.tags.join(", ")})` : "";
    return `${index + 1}. ${fact.fact}${tags} [score=${score}, hits=${hits}]`;
  }).join("\n");
}

function computeFactsFingerprint(prevFacts, importantFacts, sessions) {
  const sessionKeys = sessions.length === 0 ? ["empty"] : sessions.map((s) => `${s.session_id}:${s.updated_at}`);
  const importantKeys = importantFacts.map((fact) => `${fact.id}:${fact.importance_score}:${fact.hit_count}:${fact.last_accessed_at || ""}`);
...
export async function compileToday(summaryManager, outputPath, resolvedModel) {
  return withFileLock("compile-today", async () => {
...
export async function compileWeek(summaryManager, outputPath, resolvedModel) {
  return withFileLock("compile-week", async () => {
    fs.mkdirSync(path.dirname(outputPath), { recursive: true });
...
export async function compileLongterm(weekMdPath, longtermPath, resolvedModel) {
  return withFileLock("compile-longterm", async () => {
...
export async function compileFacts(summaryManager, outputPath, resolvedModel, opts = {}) {
  ...
  const importantFacts = factStore?.getImportantFacts
    ? factStore.getImportantFacts({ limit: 18, minImportance: factStore.compileThreshold })
    : [];
  ...
  const fp = computeFactsFingerprint(prevFacts, importantFacts, sessions);
  const fpPath = outputPath + ".fingerprint";
  try {
    if (fs.readFileSync(fpPath, "utf-8").trim() === fp && fs.existsSync(outputPath)) return "skipped";
  } catch {}
  ...
  const highImportanceSection = buildHighImportanceFactSection(importantFacts);
  const importedFacts = parseFactsMarkdown(prevFacts);
  const summaryFacts = parseFactsMarkdown(factParts.join("\n"));
  const combinedFacts = [
    ...new Set([
      ...importedFacts,
      ...summaryFacts,
  ...
  if (!highImportanceSection && combinedFacts.length === 0) {
    if (!prevFacts) atomicWrite(outputPath, "");
    fs.writeFileSync(fpPath, fp);
    return "compiled";
  }

  if (combinedFacts.length < 8 && !highImportanceSection) {
    atomicWrite(outputPath, combinedFacts.join("\n"));
    fs.writeFileSync(fpPath, fp);
    return "compiled";
  }
  ...
  const isZh = _isZh();
  const sections = [];
  if (highImportanceSection) {
    sections.push(isZh ? `## 高重要度事实\n${highImportanceSection}` : `## High-importance facts\n${highImportanceSection}`);
  }
  if (combinedFacts.length > 0) {
    sections.push(isZh ? `## 近期候选事实\n${combinedFacts.join("\n")}` : `## Recent candidate facts\n${combinedFacts.join("\n")}`);
  }

  const result = await _compactLLM(
    sections.join("\n\n"),
    isZh
      ? "将以下重要事实去重合并（220字以内）。优先保留‘高重要度事实’中的稳定信息，再用近期候选事实补充。只保留稳定的、跨时间有效的用户属性：身份、偏好、关系、习惯、长期项目背景。矛盾时以更高重要度或更新事实为准。直接输出事实列表，每行一条。"
      : "Deduplicate and merge the following key facts (under 140 words). Prioritize the stable information in High-importance facts, then use recent candidates as supporting updates. Keep only persistent, time-stable user attributes: identity, preferences, relationships, habits, and long-running project context. When facts conflict, prefer higher-importance or newer facts. Output one fact per line.",
    resolvedModel,
    340,
  );
