...
// ── File-based mutex (atomic mkdir) to prevent concurrent compile races ──
const _activeLocks = new Map(); // lockDir → Promise

async function withFileLock(name, fn) {
  const lockDir = path.join(os.tmpdir(), `.lynn-lock-${name}`);

  // If we already hold this lock in-process, queue behind it
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      // Stale lock detection: if lock dir is older than 5 min, remove it
      try {
