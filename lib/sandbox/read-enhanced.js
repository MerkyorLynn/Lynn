 ...
 *
 * 另外导出 wrapReadToolWithFuzzy：ENOENT 时 glob 搜索 basename 给模型提示，
 * 打破「模型给错路径 → ENOENT → 原样重试」死循环。
 */
...
/**
 * 当 read 返回 ENOENT 时，在 cwd 下 glob 搜索同名文件。
...
async function suggestFuzzyPath(requestedPath, cwd) {
  ...
  const base = pathBasename(requestedPath);
  if (!base || base.startsWith(".")) return null; // 避免 .env/.DS_Store 之类暴露
  try {
...
/**
 * 包装 read 工具:ENOENT 时返回模糊搜索提示而非原始错误。
    ...
    execute: async (toolCallId, params, ...rest) => {
      try {
        ...
        if (!suggestions || suggestions.length === 0) {
          // 无候选 → 把原错误包成 tool result，避免 agent 层把错误当失败重试
          return {
        ...
        const hint = suggestions.length === 1
          ? `文件不存在: ${params.path}\n\n在工作区找到一个候选 → 请用完整路径重试:\n  ${suggestions[0]}`
          : `文件不存在: ${params.path}\n\n在工作区找到 ${suggestions.length} 个候选，请用完整路径重试:\n${suggestions.map((s) => "  " + s).join("\n")}`;
        return {
