...
/** Recall of past SOPs — local, cheap, default-on. Off with LYNN_CLI_SKILL_CRYSTALLIZE=0. */
export function skillCrystallizeEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.LYNN_CLI_SKILL_CRYSTALLIZE !== "0";
}

/**
 * Distillation — the extra completion call that turns a finished task into an SOP.
 * Opt-in (LYNN_CLI_SKILL_CRYSTALLIZE=1): adding a model round-trip to the end of
 * every successful coding task is a real, every-task latency/cost the default should
 * not pay for a speculative future recall. Recall still runs by default and just
 * reads whatever the store already holds.
 */
export function skillDistillEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
...
export function tokenize(text: string): string[] {
  ...
  // Latin / digit words (unchanged: drop <3-char tokens and stopwords).
  for (const raw of lower.split(/[^a-z0-9_]+/)) {
    if (raw.length < 3 || STOPWORDS.has(raw)) continue;
    push(raw);
  }
  // CJK has no spaces, so whitespace tokenizing yields nothing for Chinese. Emit
  // character bigrams within each contiguous Han run — that gives Chinese the
  // overlap granularity English gets from words (登录/录失/失败 overlap on 登录)
  // instead of one giant unmatchable token. Staying within a run keeps punctuation
  // gaps from creating spurious cross-boundary pairs.
