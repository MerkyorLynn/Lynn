...
import { skillCrystallizeEnabled, skillDistillEnabled, buildDistillPrompt, parseDistilledSkill, recallSkills, formatSkillRecallFrame } from "../code-skill-distill.js";
...
async function runCodeTask(
  ...
  const final = await runCodeAgentLoop({
    ...
    json,
    maxSteps: stepBudget,
    workingCheckpoint: workingCheckpointEnabled(process.env, { longRun: isLongRun(args) }),
    toolCtx,
    input,
    output: errorOutput,
  ...
  // ② Crystallize: distill the trace into a reusable SOP. Opt-in (LYNN_CLI_SKILL_CRYSTALLIZE=1)
  // because it adds a model round-trip to every successful task. When on, it is still
  // gated on a verified mutation — the run actually changed code and the last auto-verify
  // passed (or no verify command applied) — so SOPs come from known-good runs only.
  if (skillDistillEnabled() && final.mutated && final.verifyClean && !final.maxStepsReached && final.text.trim()) {
    try {
