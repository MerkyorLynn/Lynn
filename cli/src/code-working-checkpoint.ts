...
export function workingCheckpointEnabled(
  env: NodeJS.ProcessEnv = process.env,
  opts: { longRun?: boolean } = {},
): boolean {
  // Default-on for long / endurance runs (where history compaction actually bites);
  // off for short tasks, where the pinned notepad would be pure overhead.
  // LYNN_CLI_WORKING_CHECKPOINT forces it either way (1 = on, 0 = off).
  if (env.LYNN_CLI_WORKING_CHECKPOINT === "0") return false;
  if (env.LYNN_CLI_WORKING_CHECKPOINT === "1") return true;
  return Boolean(opts.longRun);
}
