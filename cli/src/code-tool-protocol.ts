...
export function codeToolDefinitions(opts: { workingCheckpoint?: boolean } = {}): ChatToolDefinition[] {
  ...
  // Default-on for long runs (or LYNN_CLI_WORKING_CHECKPOINT=1): a model-curated
  // scratchpad, re-injected after compaction and immune to it. This keeps durable
  // context small in-process instead of leaning on long history.
  if (opts.workingCheckpoint ?? workingCheckpointEnabled(process.env)) {
    tools.push({
      type: "function",
      function: {
