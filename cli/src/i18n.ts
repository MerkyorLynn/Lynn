...
const STRINGS: Record<Lang, Record<string, string>> = {
  zh: {
    "tips.banner":
      '提示:Lynn -p "问题" 走本地 Brain 路由(默认 StepFun 3.7 Flash(256K 上下文,high 推理,32K 推理/生成预算),Spark Qwen 3.6 35B A3B 本地兜底)。\n' +
    ...
    "chat.help":
      ...
      "/model 查看 Brain 两段模型路由; /model stepfun|spark 切换 StepFun 3.7 Flash / Spark Qwen 3.6 35B A3B\n" +
    ...
    "providers.route.default": "StepFun 3.7 Flash → Spark Qwen 3.6 35B A3B · 经本地 Brain 路由(自动)",
    "providers.defaultNote": "默认模型: CLI 通过本地 Brain/router 先用 StepFun 3.7 Flash(256K 上下文,high 推理,32K 推理/生成预算),Spark Qwen 3.6 35B A3B 作为本地兜底。需要本地 Brain 在线;可运行 Lynn brain start 或打开 Lynn 客户端。",
    ...
    "models.note.fixed": "默认 Brain V2 顺序固定为 StepFun 3.7 Flash → Spark Qwen 3.6 35B A3B。",
  ...
  en: {
    "tips.banner":
      'Tip: Lynn -p "prompt" uses the local Brain router (StepFun 3.7 Flash first (256K context; high reasoning with a 32K reasoning/generation budget), Spark Qwen 3.6 35B A3B local fallback).\n' +
    ...
    "chat.help":
      ...
      "/model show the two Brain model choices; /model stepfun|spark switches StepFun 3.7 Flash / Spark Qwen 3.6 35B A3B\n" +
    ...
    "providers.route.default": "StepFun 3.7 Flash → Spark Qwen 3.6 35B A3B via local Brain router (auto)",
    "providers.defaultNote": "Default model: CLI uses StepFun 3.7 Flash first (256K context; high reasoning with a 32K reasoning/generation budget) through the local Brain/router, and Spark Qwen 3.6 35B A3B as the local fallback. Local Brain must be online; run Lynn brain start or open the Lynn client GUI.",
    ...
    "models.note.fixed": "Default Brain V2 order is fixed as StepFun 3.7 Flash -> Spark Qwen 3.6 35B A3B.",
