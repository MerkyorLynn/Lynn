...
describe("skill crystallization gating (recall default-on, distill opt-in)", () => {
  it("recall (skillCrystallizeEnabled) defaults on, off only with =0", () => {
    expect(skillCrystallizeEnabled({})).toBe(true);
    expect(skillCrystallizeEnabled({ LYNN_CLI_SKILL_CRYSTALLIZE: "1" })).toBe(true);
    expect(skillCrystallizeEnabled({ LYNN_CLI_SKILL_CRYSTALLIZE: "0" })).toBe(false);
  });
  it("distill (skillDistillEnabled) is opt-in via =1 (extra model call per task)", () => {
    expect(skillDistillEnabled({})).toBe(false);
    expect(skillDistillEnabled({ LYNN_CLI_SKILL_CRYSTALLIZE: "1" })).toBe(true);
    expect(skillDistillEnabled({ LYNN_CLI_SKILL_CRYSTALLIZE: "0" })).toBe(false);
  });
});
...
describe("tokenize", () => {
  ...
  it("emits Han bigrams so Chinese tasks tokenize (was ASCII-only → empty)", () => {
    const toks = tokenize("修复登录问题");
    expect(toks).toContain("修复");
    expect(toks).toContain("登录");
    expect(toks).toContain("录问");
  });
  it("keeps latin words alongside Han bigrams in mixed text", () => {
    expect(tokenize("修复登录bug")).toEqual(expect.arrayContaining(["bug", "修复", "登录"]));
  });
});
...
describe("recallSkills", () => {
  ...
  it("recalls a Chinese skill by Han-bigram overlap (ASCII tokenizer scored 0)", () => {
    const zh = [
      skill({ id: "zh", title: "修复登录失败", keywords: ["登录", "失败", "重试"] }),
      skill({ id: "other", title: "Format dates", keywords: ["date", "format"] }),
    ];
    const r = recallSkills("帮我修复登录失败的问题", zh);
    expect(r.map((s) => s.id)).toEqual(["zh"]);
  });
});
