...
describe("workingCheckpointEnabled (default-on for long runs)", () => {
  it("is off for short runs, on for long runs, with env override", () => {
    expect(workingCheckpointEnabled({})).toBe(false);
    expect(workingCheckpointEnabled({}, { longRun: false })).toBe(false);
    expect(workingCheckpointEnabled({}, { longRun: true })).toBe(true);
    // env forces either way regardless of run length
    expect(workingCheckpointEnabled({ LYNN_CLI_WORKING_CHECKPOINT: "0" }, { longRun: true })).toBe(false);
    expect(workingCheckpointEnabled({ LYNN_CLI_WORKING_CHECKPOINT: "1" }, { longRun: false })).toBe(true);
  });
});
...
describe("codeToolDefinitions opt-in gating", () => {
  ...
  it("includes it when opts.workingCheckpoint is set (long-run default), even with no env", () => {
    delete process.env.LYNN_CLI_WORKING_CHECKPOINT;
    expect(codeToolDefinitions({ workingCheckpoint: true }).map((t) => t.function.name)).toContain("update_working_checkpoint");
  });
});
